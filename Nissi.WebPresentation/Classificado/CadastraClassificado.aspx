<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/MasterPage.Master" CodeBehind="CadastraClassificado.aspx.cs" Inherits="Nissi.WebPresentation.Classificado.CadastraClassificado" %>
...
<script type="text/javascript">
    ...
    function TipoPesquisa(tvar) {
        ...
        $get('<%=txtLetraPesq.ClientID%>').value = '';
        $get('<%=txtNumeroPesq.ClientID%>').value = '';
        $get('divDesc').style.display = 'none';
        $get('divLetra').style.display = 'none';
        switch (tvar) {
            case 1: //Pesquisa por Desc
                $get('divLetra').style.display = 'block';
                break;
    ...
    function ValidaCampos(src, args) {
        var valido = true;
        if (($get('<%=rbLetra.ClientID%>').checked) && ($get('<%=txtLetraPesq.ClientID%>').value == '')) {
            valido = false;
            $get('<%=cvValida.ClientID %>').errormessage = "Favor informar a Letra.";
        }
        else if (($get('<%=rbDesc.ClientID%>').checked) && ($get('<%=txtNumeroPesq.ClientID%>').value == '')) {
            valido = false;
            $get('<%=cvValida.ClientID %>').errormessage = "Favor informar o Número.";
        }
         args.IsValid = valido;
     }
     //--------------------------------------------------------------------------------
...
<table style="margin-left: auto; width: 95%; margin-right: auto;">
    <tr>
        ...
        <td class="titulo">Cadastro de ClassificacaoFiscal</td>
    </tr>
</table>
...
<div style="text-align:center; width:100%;" id="divConsulta" runat="server">
    ...
    <table id="tblConsulta" runat="server" style="text-align:left; width:100%">
        <tr>
            ...
            <td style="width: 20%">
                <asp:RadioButton ID="rbLetra" onclick = "TipoPesquisa(1)"  
                    GroupName="filtroPesq" runat="server" Text="Letra" Checked="True" 
                    CssClass="noBorder" />
            </td>
            <td style="width: 20%">
                <asp:RadioButton ID="rbDesc" onclick ="TipoPesquisa(2)" runat="server" 
                    Text="Número" GroupName="filtroPesq" CssClass="noBorder" />
            </td>
            <td style="width: 20%">
                &nbsp;</td>
        </tr>
        <tr>
            ...
            <td colspan="3">
                <div id="divLetra"  style="display:block">
                    <asp:TextBox ID="txtLetraPesq" MaxLength="44" onkeypress="NoCaracterEspecial();KeyDownHandler();" runat="server" Height="16px" Width="146px"></asp:TextBox>
                    
                </div>
                <div id="divDesc" style="display:none">
                    <asp:TextBox ID="txtNumeroPesq" runat="server" onkeypress="OnlyNumbers();KeyDownHandler();" Height="16px" Width="600px"></asp:TextBox>
...
<asp:Panel ID="pnlIncluirForn" runat="server">
    <asp:UpdatePanel  runat="server" ID="upCadastro" UpdateMode="Conditional">
        <ContentTemplate>
        <asp:HiddenField runat="server" ID="hdfTipoAcao" /><asp:HiddenField ID="hdfCadastroPopup" runat="server" />
            <div style="text-align: center;width:900px; height:200px; padding: 5px 5px 5px 5px; background-color: #ffffff; overflow:auto">
                ...
                <table class="fundoTabela">
                    ...
                    <tr>
                        <td class="titulo">
                            <b><% if (hdfTipoAcao.Value == "Incluir") { %>::Inclusão de ClassificacaoFiscal
                            <% } %><% else { %>::Alteração de ClassificacaoFiscal <% } %></b>
                        </td>
                    </tr>
                </table>
                <br style="line-height:5px" />
                <table width="100%" align="center" class="fundoTabela" >
                    <tr> 
                        <td>
                            <table cellpadding="3" cellspacing="0" class="fundoTabela" style="text-align:left">
                                ...
                                <tr>
                                    <td style="padding-left:20px">Letra</td>
                                    <td style="padding-left:20px">Número</td>
                                </tr>
                                <tr>
                                  <td>
                                        <asp:TextBox ID="txtLetra"  MaxLength="44" onkeypress="NoCaracterEspecial();" runat="server" CssClass="formNovo" TabIndex="2" Width="300px"></asp:TextBox>
                                        <asp:RequiredFieldValidator ID="rfvLetra" ErrorMessage="Letra não informada!" ValidationGroup="Inserir" ControlToValidate="txtLetra" runat="server" Text="*" CssClass="asterisco"></asp:RequiredFieldValidator>
                                    </td>                                   
                                    <td>
                                        <asp:TextBox ID="txtNumero" onkeypress="OnlyNumbers();" runat="server" CssClass="formNovo" MaxLength="16" TabIndex="2" Width="300px"></asp:TextBox>
                                        <asp:RequiredFieldValidator ID="rfvNumero" ErrorMessage="Número não informada!" ValidationGroup="Inserir" ControlToValidate="txtNumero" runat="server" Text="*" CssClass="asterisco"></asp:RequiredFieldValidator>
                                    </td>
                                </tr>
                            </table>
                            <br style="line-height:5px" />
                    ...
                    <asp:HiddenField ID="hdfCodClassificacaoFiscal" runat="server" />
                </table>
            </div>
        </ContentTemplate>
        <Triggers>
